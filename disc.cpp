...
//
	...
	GLUI_EditText* editBlend;  //Group 2 add
...
double distanceFromPath(vector<Box*>&);
double minClearanceFromPath(vector<Box*>&);
...
printf("%s\n","get" );
	...
	if (noPath) {
		...
		dist = distanceFromPath(path);
		minClear = minClearanceFromPath(path);
	}

	if (interactive ==2) {
		//Print Out Run characteristics
		cout << fileName << ",";
		cout << QType << ",";
		cout << R0 << ",";
		cout << epsilon << ",";
		...
		cout << t.getElapsedTimeInMilliSec() << ","; //time elapsed
		cout << dist << ","; //distance traveled
		cout << minClear << ","; //minimumClearnace
		cout << ct << ","; // number of expansions
		cout << freeCount << ","; // free boxes
		cout << stuckCount << ","; // stuck boxes
		cout << mixSmallCount << ","; //mixed smaller then epsilon
		cout << mixCount - ct - mixSmallCount << ","; //mixed bigger then epsilon
		cout << "\n"; //mixed smaller then epsilon
...
//added by Group 2
double distanceFromPath(vector<Box*>& path)
{
	double dist = 0.0;
	double lastX = path[0]->x;
	double lastY = path[0]->y;
	for (int i = 0; i < (int)path.size(); ++i)
	{
		dist = dist + sqrt( (path[i]->x-lastX)*(path[i]->x-lastX) + 
			(path[i]->y-lastY)*(path[i]->y-lastY) );
		lastX = path[i]->x;
		lastY = path[i]->y;
	}
	return dist;
...
//added by Group 2
double minClearanceFromPath(vector<Box*>& path)
{
	double minClearance = 1000000.0;
	for (int i = 0; i < (int)path.size(); ++i)
	{
		if (path[i]->clearance < minClearance)
			minClearance = path[i]->clearance;
	}
	return minClearance;
}
